...
import { detailAction } from "@/pr-checkout/model";
...
export class DetailView {
    ...
    render() {
        ...
        const shell = h("main", { class: `pr-modal pr-detail${this.refreshing ? " pr-is-refreshing" : ""}${this.busy ? " pr-modal-busy" : ""}`, tabindex: -1 }, refreshBar(this.refreshing, "Refreshing pull request details"), heading(`Pull request #${this.pr.number}`, `${this.pr.headBranch || "head"} → ${this.pr.baseBranch || "base"}`, "pr", this.onBack), content, actions, footer(label, [["⇧↵", "Open"], ["⌘↵", "Merge"], ["⌘⌫", "Close"], ["⌘R", "Reload"], ["←", "Back"]]));
        clear(this.root);
    ...
    content(detail) {
        ...
        return h("article", { class: "pr-detail-content" }, h("section", { class: "pr-detail-overview" }, h("div", { class: "pr-detail-kicker" }, h("span", { class: `pr-detail-state${detail.isDraft ? " pr-detail-state-draft" : ""}` }, icon(detail.isDraft ? "circleDashed" : "pr", 11, "", 2), detail.isDraft ? "Draft" : "Open"), h("span", {}, `#${detail.number}`)), h("h2", {}, detail.title || `Pull request #${detail.number}`), h("p", { class: "pr-detail-byline" }, `Opened by ${detail.author || "Unknown author"}${dateLabel(detail.createdAt) ? ` · ${dateLabel(detail.createdAt)}` : ""}`), h("div", { class: "pr-detail-ledger" }, stat("files", detail.changedFiles ?? 0), stat("added", `+${detail.additions ?? 0}`, "pr-stat-add"), stat("removed", `−${detail.deletions ?? 0}`, "pr-stat-remove"), stat("comments", comments.length))), h("section", { class: "pr-detail-section" }, h("h3", {}, "Summary"), summary), h("section", { class: "pr-detail-section pr-comments" }, h("h3", {}, `Comments · ${comments.length}`), comments.length ? comments.map((comment) => this.comment(comment, detail.url)) : h("p", { class: "pr-detail-empty" }, "No comments yet.")));
    }
    ...
    actions() {
        return h("div", { class: "pr-detail-actions", "aria-label": "Pull request actions" }, ACTIONS.map((action) => h("button", {
            ...
            disabled: this.busy || this.refreshing || (action.id === "open" && !this.detail?.url),
